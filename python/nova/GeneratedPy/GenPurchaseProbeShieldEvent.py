'''
GenPurchaseProbeShieldEvent.py
...
class GenPurchaseProbeShieldEvent(PurchaseEvent):
	...
	def construct(self, game, executionTime, player, cost, star):
		# Construct the PurchaseProbeShieldEvent with all the required attributes that do not have default values
		self.setGame( game )
	...
	def star(self):
		if self._star is not None and type(self._star) is not InstanceType:
			try:
				...
				self.__dict__['_star'] = self.objRefErrorWasRaised(e, 'PurchaseProbeShieldEvent', 'star')
		return self._star

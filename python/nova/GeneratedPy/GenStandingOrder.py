...
class GenStandingOrder(GameObject):
	...
	def construct(self, game, star):
		# Construct the StandingOrder with all the required attributes that do not have default values
		self.setGame( game )
		self.setStar( star )
	...
	def star(self):
		if self._star is not None and type(self._star) is not InstanceType:
			try:
				...
				self.__dict__['_star'] = self.objRefErrorWasRaised(e, 'StandingOrder', 'star')
		return self._star

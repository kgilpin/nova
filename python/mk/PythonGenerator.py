	...
	def writePyInit(self):
		...
		for attr in self.requiredAttrs():
			if attr.defaultValue() is None:
				wr(', %s' % attr.name())
		wr('):\n')
		wr('\t\t# Construct the %s with all the required attributes that do not have default values\n' % self.name())

		if len(self.requiredAttrs()) > 0:
			for attr in self.requiredAttrs():
				if attr.defaultValue() is None:
					wr('\t\tself.%s( %s )\n' % ( attr.pySetName(), attr.name()) )
		else:
			wr('\t\tpass')
...
''')
	...
	def requiredAttrs(self):
		list = []
		self._findRequiredAttrs(self, list)
		return list

	def _findRequiredAttrs(self, klass, list):
		if klass.superklass() is not None:
			self._findRequiredAttrs(klass.superklass(), list)

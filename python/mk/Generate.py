...
class Generate:
	...
	def main(self, args=sys.argv):
		...
		if opt.has_key('value'):
			print 'Generating Python value objects...'
			self.generate(
				pyClass='ValueObjectGenerator',
				model=opt['model'],
				outdir=outdir)
		if opt.has_key('java'):
			print 'Generating Java...'
			self.generate(
				pyClass='JavaGenerator',
				model=opt['model'],
				outdir=outdir)
	...
	def usage(self, errorMsg=None):
		...
		print 'Usage: %s --db DBNAME --model FILENAME [--sql] [--py] [--value] [--java] [--outdir DIRNAME]' % progName
	...
	def options(self, args):
		...
		if type(args)==type(''):
			args = args.split()
		optPairs, files = getopt(args[1:], 'h', ['help', 'db=', 'model=', 'sql', 'py', 'value', 'java', 'outdir='])
		if len(optPairs)<1:
